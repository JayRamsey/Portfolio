...
class Library:
    ...
    def search_book(self, title): return f"Title: {self.books[title].title}, Author: {self.books[title].author}, Genre: {self.books[title].genre}" if title in self.books else f"No books with title '{title}'"
...
def main():
    ...
    removeButton = tk.Button(removeMenu, text="Remove", command=lambda: library.remove_book(removeTitleEntry.get() if removeTitleEntry.get() in library.books else False))
